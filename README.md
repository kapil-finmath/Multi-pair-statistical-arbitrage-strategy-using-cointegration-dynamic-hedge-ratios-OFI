...
This project develops a complete statistical arbitrage framework for exchange-traded funds (ETFs) by combining classical econometric techniques, stochastic mean-reversion models, dynamic hedge ratio estimation, and market microstructure signals.

The research begins by identifying cointegrated ETF pairs using the Engle-Granger and Johansen cointegration frameworks. The long-run equilibrium relationship between assets is then modeled through a stationary spread process. Mean-reversion dynamics are quantified using an Ornstein-Uhlenbeck (OU) process, allowing estimation of reversion speed and half-life.
